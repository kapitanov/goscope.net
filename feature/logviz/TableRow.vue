...
const contextualTextClass = computed(() => {
  ...
  return {
    'text-balance': true,
    'text-red-900': ERROR_LEVELS.includes(level),
    'text-yellow-700': WARNING_LEVELS.includes(level)
  };
});
</script>
      ...
      <div class="grow">
        ...
        <ul class="border-s-2 border-cyan-900 mt-2">
          <li v-if="item.timestamp" class="flex flex-col ms-2">
            <span class="text-cyan-900 text-balance break-all">timestamp</span>
            <span class="ms-4 text-balance break-all">{{ item.timestamp }}</span>
          </li>
          <li v-if="item.logger" class="flex flex-col ms-2">
            <span class="text-cyan-900 text-balance break-all">logger</span>
          ...
          <li v-for="field in item.fields" :key="field.key" class="flex flex-col ms-2">
            <span class="text-cyan-900 text-balance break-all">{{ field.key }}</span>
