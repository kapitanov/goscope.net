...
const contextualTextClass = computed(() => {
  ...
  return {
    'text-balance': true,
    'text-green-900 dark:text-green-400': isRunnable,
    'text-red-900 dark:text-red-400': !isRunnable
  };
});
</script>
  ...
  <div class="flex flex-col gap-1 px-2 py-1 font-mono border-b border-gray-200 dark:border-gray-700">
    <div class="flex flex-row gap-2">
      <div class="w-6 shrink-0">
        <a href="" class="block hover:text-cyan-700 dark:hover:text-cyan-400" @click.prevent="toggleHandler">
      ...
      <div class="hidden md:block grow">
        <CopyableText>
          <a
            href=""
            class="grow-1 hover:text-cyan-700 dark:hover:text-cyan-400 hover:underline decoration-cyan-700 text-balance text-green-900 dark:text-green-400 break-all"
      ...
      <div class="grow">
        ...
        <ul class="border-s-2 border-cyan-900 dark:border-cyan-600 mt-2">
          <li v-for="frame in item.stack" :key="frame.id" class="flex flex-col ms-2">
            <span class="text-cyan-900 dark:text-cyan-400 text-balance break-all">
              {{ frame.function }}
            </span>
