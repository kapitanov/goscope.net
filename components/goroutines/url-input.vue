...
<template>
  ...
  <form class="flex flex-col gap-2" @submit.prevent="goHandler">
    <div>
      Paste an URL of your PPROF goroutines endpoint into a text field below and
      click "Go!" to visualize it.
    </div>
    <div>
      It works event with localhost URLs (assuming you have correct CORS policy
      on your service's side)!
    </div>
    ...
    <ul class="grow text-gray-500 text-sm">
      <li>
        We will fetch this URL locally, so it must be accessible from your
        computer. No data will be sent to our servers.
      </li>
      <li v-if="!validationError && urlHint" class="text-yellow-700">
        Don't forget to add <code>?debug=2</code> to your URL.
      </li>
